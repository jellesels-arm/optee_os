...
static bool is_nw_buf(struct ffa_rxtx *buf)
{
	return buf && buf == &nw_rxtx;
}
...
void spmc_handle_rxtx_unmap(struct thread_smc_args *args, struct ffa_rxtx *buf)
{
	...
	if (is_nw_buf(buf)) {
		unmap_buf(buf->rx, buf->size);
		unmap_buf(buf->tx, buf->size);
	}
	buf->size = 0;
	buf->rx = NULL;
	buf->tx = NULL;
				      ...
				      struct ffa_rxtx *buf)
{
	...
	cpu_spin_lock(&buf->spinlock);
	if (buf->size && buf->tx_is_mine) {
		struct ffa_partition_info *fpi = buf->tx;
...
static int handle_mem_share_tmem(paddr_t pbuf, size_t blen, size_t flen,
				 unsigned int page_count,
				 uint64_t *global_handle, struct ffa_rxtx *buf)
{
	int rc = 0;
	size_t len = 0;
	tee_mm_entry_t *mm = NULL;
				  ...
				  uint64_t *global_handle, struct ffa_rxtx *buf)
{
	...
	if (buf->rx && flen <= buf->size)
		rc = add_mem_share(NULL, buf->rx, blen, flen, global_handle);
...
static void handle_mem_share(struct thread_smc_args *args, struct ffa_rxtx *buf)
{
	uint32_t ret_w1 = 0;
